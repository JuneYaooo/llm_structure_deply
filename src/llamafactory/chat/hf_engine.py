...
from src.llamafactory.extras.misc import get_logits_processor
...
if TYPE_CHECKING:
    ...
    from src.llamafactory.data import Template
...
class HuggingfaceEngine(BaseEngine):
    ...
    @staticmethod
    def _batch_process_args(
        model: "PreTrainedModel",
        tokenizer: "PreTrainedTokenizer",
        processor: Optional["ProcessorMixin"],
        template: "Template",
        generating_args: Dict[str, Any],
        batch_messages: Sequence[Dict[str, str]],
        system: Optional[str] = None,
        tools: Optional[str] = None,
        image: Optional["NDArray"] = None,
        input_kwargs: Optional[Dict[str, Any]] = {},
        ...
        for messages in batch_messages:
            ...
            if processor is not None and image is not None:  # add image features
                ...
                if hasattr(processor, "image_seq_length"):  # paligemma models
                    image_token_id = tokenizer.convert_tokens_to_ids(template.image_token)
                    prompt_ids = [image_token_id] * getattr(processor, "image_seq_length") + prompt_ids
            inputs.append({"input_ids": prompt_ids, "attention_mask": [1] * len(prompt_ids)})

        
        # inputs = torch.tensor([prompt_ids], device=model.device)
        # attention_mask = torch.ones_like(inputs, dtype=torch.bool)
        batch_input = tokenizer.pad(
            inputs, return_attention_mask=True, padding=True, return_tensors="pt"
        ).to(model.device)
        prompt_length = len(batch_input["input_ids"][0])
        do_sample: Optional[bool] = input_kwargs.pop("do_sample", None)
        temperature: Optional[float] = input_kwargs.pop("temperature", None)
        top_p: Optional[float] = input_kwargs.pop("top_p", None)
        top_k: Optional[float] = input_kwargs.pop("top_k", None)
    ...
    def _batch_chat(
        ...
        gen_kwargs, prompt_length = HuggingfaceEngine._batch_process_args(
            model, tokenizer, processor, template, generating_args, batch_messages, system, tools, image, input_kwargs
        )
        generate_output = model.generate(**gen_kwargs)
        response_ids = generate_output[:, prompt_length:]
        response = tokenizer.batch_decode(response_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True)
    ...
    async def batch_chat(
        ...
        **input_kwargs,
        ...
        async with self.semaphore:
            with concurrent.futures.ThreadPoolExecutor() as pool:
                return await loop.run_in_executor(pool, self._batch_chat, *input_args)
